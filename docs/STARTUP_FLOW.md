...
| `2` | `Game` | `GM` | `Inner.ProcessRegister (1000)` | 已登记消息 | `Game` 建立到 `GM` 的 `Inner` 主动链路后，注册自身 `NodeID`、`innerNetworkEndpoint`、版本与初始负载 |
| `3` | `GM` | `Game` | `Inner.ProcessRegister (1000)` response | 已登记消息 | `GM` 接受或拒绝 `Game` 注册；成功时返回心跳参数 |
| `4` | `Game` | `GM` | `Inner.ProcessHeartbeat (1100)` | 已登记消息 | `Game` 在注册成功后进入周期心跳 |
| `5` | `GM` | `Game` | `Inner.ProcessHeartbeat (1100)` response | 已登记消息 | `GM` 确认 `Game` 当前注册仍然有效 |
| `6` | `Gate` | `GM` | `Inner.ProcessRegister (1000)` | 已登记消息 | `Gate` 建立到 `GM` 的 `Inner` 主动链路后，注册自身 `NodeID`、`innerNetworkEndpoint`、版本与初始负载 |
| `7` | `GM` | `Gate` | `Inner.ProcessRegister (1000)` response | 已登记消息 | `GM` 接受或拒绝 `Gate` 注册；成功时返回心跳参数 |
| `8` | `Gate` | `GM` | `Inner.ProcessHeartbeat (1100)` | 已登记消息 | `Gate` 在注册成功后进入周期心跳 |
| `9` | `GM` | `Gate` | `Inner.ProcessHeartbeat (1100)` response | 已登记消息 | `GM` 确认 `Gate` 当前注册仍然有效 |
...
| `12` | `Game` | `Gate` | `Inner.ProcessRegister (1000)` | 已登记消息 | `Game` 向目标 `Gate` 发起注册，声明自己可为该 `Gate` 提供内部转发目标 |
| `13` | `Gate` | `Game` | `Inner.ProcessRegister (1000)` response | 已登记消息 | `Gate` 接受或拒绝该 `Game` 的链路注册；成功时返回心跳参数 |
| `14` | `Game` | `Gate` | `Inner.ProcessHeartbeat (1100)` | 已登记消息 | `Game` 在注册成功后对该 `Gate` 进入周期心跳 |
| `15` | `Gate` | `Game` | `Inner.ProcessHeartbeat (1100)` response | 已登记消息 | `Gate` 确认该 `Game` 针对自己的注册仍然有效 |
...
5. `Gate -> GM: Inner.ProcessRegister (1000)`
6. `GM -> Gate: 1000 response`
7. `Gate -> GM: Inner.ProcessHeartbeat (1100)`
8. `GM -> Gate: 1100 response`
9. `GM -> Game: Inner.ServerStubOwnershipSync (1202)`
10. `Game -> Gate[*]: Inner.ProcessRegister / Inner.ProcessHeartbeat`

...
**适用范围**
1. 适用于仓库顶层目录、`3rd`、`client`、`src/native`、`src/managed`、`docs`、`configs`。
...
4. `client/`：放置独立客户端侧代码、脚本与客户端测试；它与 `src/managed/` 的服务端 managed 项目职责分离。
5. `src/`：放置所有源码、项目文件与与源码强关联的构建描述。
6. `3rd/`：放置 vendored 第三方依赖；每个依赖单独使用子目录，保存上游源码与必要的项目侧构建包装。
7. `logs/`：运行期生成目录，默认用于日志输出，不应提交到仓库。
8. 顶层新增目录必须满足“无法归入现有目录”的前提，并同步更新 `docs/PROJECT.md`、本规范以及相关依赖说明。
...
**Client 目录与命名规则**
1. `client/` 用于独立客户端或工具型可执行项目，不承载服务端 managed 运行时入口、`Game` 业务程序集或服务端共享测试项目。
2. `client/` 下的可执行项目目录使用 `PascalCase`，推荐以产品语义命名，例如 `XServer.Client`。
3. 客户端项目默认使用 `XServer.Client` 作为根命名空间前缀；如存在多个客户端工具，可继续扩展为 `XServer.Client.<ToolName>`。
4. 客户端测试项目统一放在 `client/Tests/` 下，并采用 `<ProjectName>.Tests` 命名。
...
**可失败 API 返回约定**
1. 项目自有的公开可失败 API 优先直接返回错误码类型；成功固定返回 `0` / `None`，失败返回具有稳定语义的非零错误码。
2. 当函数还需要返回数据、句柄、视图、长度或新创建对象时，统一通过输出参数或结果结构承载，不再混用 `bool + error_message`、`值或负错误码` 这类双通道返回。
3. `bool` 仅用于纯谓词、状态查询或模块内部不对外暴露的辅助操作；如果失败原因需要被调用方区分，则不应继续使用 `bool` 作为公开返回值。
4. 每个公开错误码类型都应提供稳定的错误消息映射函数，并保持“成功为 `0`，失败为非 `0`”的数值语义；跨 C++/C# 边界时统一映射到 `int32` 兼容表示。
5. 旧接口在完成迁移前可以暂时保留兼容层；当前 `TimerManager` 创建接口仍沿用“成功返回 `TimerID`、失败返回负错误码”的历史约定，但新增模块与新增公开接口不得继续复制这一模式。

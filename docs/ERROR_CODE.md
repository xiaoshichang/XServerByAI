...
| `3000-3999` | 控制面 / 路由 | GM 注册、心跳、配置下发、路由与内部 RPC 相关失败 |
...
| `20000-29999` | Room 业务 | 房间实体与房间域业务失败 |
...
| `50000+` | 预留 | 后续多机扩展、大版本迁移或新增系统域 |

**当前预留子段**

| 范围 | 预留对象 | 说明 | 关联条目 |
| --- | --- | --- | --- |
| `1-99` | 参数与输入校验 | 缺字段、非法参数、越界、格式错误 | `M2-13`, `M5-16` |
| `100-199` | 状态与冲突 | 状态不匹配、重复操作、资源忙、版本冲突 | `M2-13`, `M5-16` |
| `200-299` | 超时与限流 | 超时、取消、频率限制、重试耗尽 | `M2-13`, `M4-13` |
...
| `2100-2199` | 会话与鉴权 | 会话不存在、会话过期、鉴权失败、客户端超时 | `M4-03`, `M4-04`, `M4-15` |
| `3000-3099` | 注册与心跳 | 进程未注册、重复注册、心跳超时、实例不可用 | `M1-09`, `M3-04`, `M3-05` |
| `3100-3199` | 路由与转发 | 无路由、路由失效、内部 RPC 超时、转发失败 | `M1-10`, `M4-06`, `M4-13` |
...
| `20000-20999` | Room 实体核心 | 房间不存在、房间已满、状态不允许操作 | `M5-07`, `M5-09`, `M5-16` |
...
3. 模块专属错误应使用与责任域一致的前缀，例如 `Protocol.BadMagic`、`Relay.RouteNotFound`、`Interop.RuntimeLoadFailed`、`Player.NotFound`、`Room.Full`。

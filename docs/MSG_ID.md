...
| `1200-1299` | 配置 / 路由下发 | GM 下发配置、Game 路由目录、列表快照与路由变更 | `M3-06`, `M3-12`, `M3-15` |
...
其中会话与路由相关消息在落具体结构时，应复用 `docs/SESSION_ROUTING.md` 中的 `sessionId`、`playerId`、`gameInstanceId`、`gameRegistrationId` 与 `routeEpoch` 语义，避免为同一条路由关系创造多套字段命名。

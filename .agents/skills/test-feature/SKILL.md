...
3. Run `scripts/build-native.ps1` from this skill before concluding the test. It is the default native build verification for every feature test; if `build/` is missing or unconfigured, report that and stop.
...
- `scripts/build-native.ps1`
  Run this script for every feature test. It verifies that `build/` contains a configured CMake build tree, then runs `cmake --build .` from `build/`.
